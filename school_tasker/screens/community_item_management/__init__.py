    ...
    async def add_default_keyboard(self, update, context):
        ...
        if db_length > 0:
            ...
            for i in range(db_length):
                ...
                rod_name_list = await backend.get_rod_name_of_class_item(context, main_name)
                rod_name = get_clean_var(rod_name_list, 'to_string', 0, True)
                groups_list = await backend.get_group_of_class_item(context, main_name)
                groups = get_clean_var(groups_list, 'to_string', 0, True)
                index = await backend.get_item_index_of_class_item(context, main_name)
                index = get_clean_var(index, 'to_string', 0, True)
                keyboard.append(
