...
import backend
from captions import BUTTON_BACK, WHAT_YOU_WANT_TO_CHANGE_IN_THIS_TASK
...
class SchoolTaskChangeBase(base_screen.BaseScreen):
    ...
    async def add_default_keyboard(self, update, context):
        from school_tasker.screens import school_task_change_group_number, school_task_change_main
        ...
        groups = await backend.get_groups_by_name(context, check_item)
        groups = get_clean_var(groups, 'to_string', 0, True)
        if int(groups) > 1:
            context.user_data['ADDING_TASK_GROUPS'] = groups
            keyboard.insert(2, [
    ...
    async def change_task_day(self, update, context):
        ...
        context.user_data['ADDING_TASK_TASK_MONTH'] = await backend._execute_query('SELECT task_month FROM ' + context.user_data['CURRENT_CLASS_NAME'] + '_Tasks WHERE item_index = %s',
                                                                                   (context.user_data['ADDING_TASK_INDEX'],))
        context.user_data['ADDING_TASK_TASK_MONTH'] = get_clean_var(context.user_data['ADDING_TASK_TASK_MONTH'], 'to_string', 0, True)
    ...
    async def change_school_task(self, update, context):
        ...
        return await school_task_change_task.SchoolTaskChangeTask().move_along_route(update, context)
