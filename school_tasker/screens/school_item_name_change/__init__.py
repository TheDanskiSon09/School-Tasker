...
import backend
from captions import ENTER_NEW_ITEM_NAME, ITEM_NAME_WAS_SUCCESSFULLY_CHANGED, CHANGE_ITEM_NAME_AGAIN, \
...
from school_tasker.screens import school_item_change_base_class, school_item_management, community_management_main, \
    main_menu
from states import CHANGING_ITEM_NAME


class SchoolItemNameChange(school_item_change_base_class.SchoolItemChangeBaseClass, RouteMixin):
    routes = (
        ({DEFAULT_STATE}, CHANGING_ITEM_NAME),
    )
    description = ENTER_NEW_ITEM_NAME
    ...
    async def go_change_item_name(self, update, context):
        return await SchoolItemNameChange().move_along_route(update, context)
