...
import backend
from constants import BUTTON_BACK_TO_MENU, SELECT_AVAILABLE_COMMUNITY
from school_tasker.screens.base import base_screen
from utils import get_clean_var, get_payload_safe


class CurrentCommunityChange(base_screen.BaseScreen):
    description = SELECT_AVAILABLE_COMMUNITY

    async def add_default_keyboard(self, update, context):
        from school_tasker.screens import main_menu
        keyboard = []
        name_list = await backend.execute_query('SELECT class_name FROM UserCommunities WHERE user_id = %s', (update.effective_user.id,))
        db_length = await backend.execute_query('SELECT COUNT(*) FROM UserCommunities WHERE user_id = %s', (update.effective_user.id,))
